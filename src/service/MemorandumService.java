...
public class MemorandumService {
	...
	public int saveMemorandum(Memorandum memo) {
		...
		String sql = "INSERT INTO memorandum (memo_id,memo_xqzs,memo_ylzs,memo_title"
				+ ",memo_content,memo_year,memo_month,memo_day,username) " + "VALUES (?,?,?,?,?,?,?,?,?)";

		return jdbc.update(sql, memo.getMemo_id(), memo.getMemo_xqzs(), memo.getMemo_ylzs(), memo.getMemo_title(),
				memo.getMemo_content(), memo.getMemo_year(), memo.getMemo_month(), memo.getMemo_day(),
				memo.getUsername());
	}
	...
	public Map<String, Object> selectMemorandum(String username) {
		...
		String sql = "SELECT memo_id as id,memo_xqzs as xqzs,memo_ylzs as ylzs"
				...
				+ ",memo_month as month,memo_day as day FROM memorandum " + "WHERE username = ?";
